...
/* harmony default export */ var objectImage = ({
  ...
  methods: {
    ...
    shareImageToCommon: function shareImageToCommon(id) {
      var _this3 = this;
      ...
      this.$confirm('将此项目镜像共享到公共镜像库？', '提示', {
        ...
        _this3.$axios.post('/shareToPublic', { image_id: id }).then(function (res) {
          _this3.$message({
            type: 'success',
            message: '共享成功'
          });
    ...
    initMsg: function initMsg() {
      ...
      this.$axios.get(this.tableData.url, { params: lin }).then(function (res) {
        ...
        var tableDate = res.data.data.image_list;
        // 数据处理
        _this4.tableData.msg = [].concat(toConsumableArray_default()(tableDate)); // 深度拷贝
        console.log(_this4.tableData.msg);
        // 重置数据总数
        if (_this4.tableData.Pagination !== undefined) {
          _this4.tableData.Pagination.total_count = res.data.data.total_count;
        }
        _this4.tableData.loading = false;
      });
    },
    deleteRequest: function deleteRequest(id) {
      ...
      this.$confirm('确定要删除？', '提示', {
        ...
        _this5.$axios.post('/delete-image', { image_id: id }).then(function (res) {
          _this5.$message({
            type: 'success',
            message: '删除成功'
          });
    ...
    changeImage: function changeImage(id) {
      if (isNaN(id)) {
        this.imageId = '';
        this.$router.push({ name: 'editImage', query: { hasObjectData: true } });
      } else {
        this.imageId = id;
        this.$router.push({ name: 'editImage', query: { imageId: id, hasObjectData: true } });
      }
    }
  },
  components: {
